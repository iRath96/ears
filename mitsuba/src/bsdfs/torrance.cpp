...
#define REPLACE_OLDBECKMANN 1
#define STRICT_PARAMETERS 1
...
public:
	Torrance(const Properties &props) : BSDF(props) {
	...
	Torrance(Stream *stream, InstanceManager *manager)
	 : BSDF(stream, manager) {
	...
	float Fresnel(float cosThetaI_)const{

		float g;
		float c = cosThetaI_;
		g = sqrt(m_eta*m_eta-1.0f+(c*c));
		float f = 0.5f*(pow(g-c,2.0f)/pow(g+c,2.0f));
		f*=(1.0f+(pow(c*(g+c)-1.0f,2.0f)/pow(c*(g-c)+1.0f,2.0f)));
		return f;
	...
	void configure() {
		//m_model = EBSDFModel::EMTorrance;
		bool constAlpha = m_alpha->isConstant();

#if STRICT_PARAMETERS
    if(m_alpha->getMaximum().isZero()) {
      Log(EError, "Don't use this model (alpha == 0.0), Use the smooth one");
    }
#endif
		...
		if (sAvg > 1e-6f){
			m_components.push_back(EGlossyReflection | EFrontSide |
				...
				|| !m_alpha->isConstant()) ? ESpatiallyVarying : 0));
		}
		if (dAvg > 1e-6f){
...
//		}
	...
	Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		if (hasSpecular) {
			...
			const Spectrum k(0.0f);
			const Spectrum eta(m_eta);
			const Float F = fresnelConductorExact(dot(bRec.wi, H), eta,k).average();
			//const Float F = Fresnel(dot(bRec.wi, H));
...
//
//	Float pdfFull(const BSDFSamplingRecord &bRec, EMeasure measure) const {
//		bool hasSpecular = (bRec.typeMask & EGlossyReflection) &&
//			(bRec.component == -1 || bRec.component == 0);
//		bool hasDiffuse = (bRec.typeMask & EDiffuseReflection) &&
//			(bRec.component == -1 || bRec.component == 1);
...
//
//			const Float prob = distr.pdf(bRec.wi, H);
//			SLog(EInfo, "sampleVisible: %d \t prob: %f \t dw: %f", (int)distr.getSampleVisible(), prob, dwh_dwo);
//			//result = prob * dwh_dwo * probSpecular;
//			result = prob*probSpecular; // *dwh_dwo;
//		}
//
//		if (hasDiffuse)
//			result += probDiffuse * warp::squareToCosineHemispherePdf(bRec.wo);
...
//	}
	...
	Spectrum sample(BSDFSamplingRecord &bRec, const Point2 &sample) const {
		Float pdf;
		return Torrance::sample(bRec, pdf, sample);
	}
    ...
    if(Frame::cosTheta(bRec.wi) <= 0) {
      pdf = 0; // < No possible sampling
      return -1;
    }

	Float alpha = m_alpha->eval(bRec.its).average();
    if(alpha > roughtConst) {
      // This will leads to the same decision
      // for all component
      pdf = 1;
      return -1;
    }
	  ...
	  /* Find the probability of sampling the specular component */
    //Float probSpecular = 1 - m_externalRoughTransmittance->eval(Frame::cosTheta(bRec.wi), alpha);
    ...
    if (sample.y < probSpecular) {
      pdf = probSpecular;
      sample.y = sample.y*probSpecular;  // scale sample
      return 0;
    } else {
      pdf = (1-probSpecular);
      sample.y = (sample.y-probSpecular)/(1-probSpecular);  // scale sample
      return 1;
    }
  }
 ...
 */
class TorranceShader : public Shader {
public:
	TorranceShader(Renderer *renderer, const Texture *specularReflectance,
...
Shader *Torrance::createShader(Renderer *renderer) const {
	return new TorranceShader(renderer,
		m_specularReflectance.get(), m_diffuseReflectance.get(),
		m_alpha.get(), m_eta);
}

MTS_IMPLEMENT_CLASS(TorranceShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(Torrance, false, BSDF)
MTS_EXPORT_PLUGIN(Torrance, "Torrance BRDF");
MTS_NAMESPACE_END
